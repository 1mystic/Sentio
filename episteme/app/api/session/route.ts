...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    // Fetch fingerprint, learner profile, and insight card in parallel
    const [{ data: fingerprint }, { data: learnerProfile }, { data: insightCard }] = await Promise.all([
      supabase.from('session_fingerprints').select('*').eq('session_id', id).single(),
      supabase.from('learner_profiles').select('*').eq('session_id', id).single(),
      supabase.from('insight_cards').select('*').eq('session_id', id).order('created_at', { ascending: false }).limit(1).single(),
    ])
