...
function buildMarkdown(params: {
  ...
  // Fallback: when no concept rows exist, derive depth from clarity score
  if (Object.values(depthCounts).every((v) => v === 0)) {
    if (clarityScore >= 80) depthCounts['SYNTHESIS'] = 1
    else if (clarityScore >= 60) depthCounts['ANALYTICAL'] = 1
    else if (clarityScore >= 40) depthCounts['CONCEPTUAL'] = 1
    else depthCounts['SURFACE'] = 1
  }
  ...
  lines.push(`- **Concepts explored:** ${concepts.length > 0 ? concepts.length : 'tracked via session'}`)
