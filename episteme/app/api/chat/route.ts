...
import { updateSessionFingerprint } from '@/lib/fingerprint'
import type { SocraticState } from '@/lib/types'
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    // Priority: concept row → session_state backup → domain priors
    const domainPrior: BKTState = DOMAIN_BKT_PRIORS[domain] ?? DOMAIN_BKT_PRIORS.general
    const sessionBKT: BKTState | null = sessionState.bktPL != null ? {
      pL: sessionState.bktPL!,
      pT: sessionState.bktPT ?? domainPrior.pT,
      pS: sessionState.bktPS ?? domainPrior.pS,
      pG: sessionState.bktPG ?? domainPrior.pG,
    } : null
    let currentBKT: BKTState = sessionBKT ?? domainPrior
    ...
    try {
      ...
      if (conceptRow?.bkt_pL != null) {
        currentBKT = {
          pL: conceptRow.bkt_pL,
      ...
      // use session_state BKT backup or domain priors (already set above)
    }
        ...
        try {
          ...
          try {
            ...
            if (latestConcept) {
              ...
              // classify() failed to insert the concept row — create it now so BKT is never lost
              const conceptName = (conceptsCovered[0] ?? concept).slice(0, 80).replace(/[?!.]+$/, '').trim()
              await supabase.from('concepts').insert({
                ...
                bkt_pL: newBKTState.pL,
                bkt_pT: newBKTState.pT,
                bkt_pS: newBKTState.pS,
                bkt_pG: newBKTState.pG,
              })
            }
            ...
            // Non-blocking fingerprint update
            updateSessionFingerprint(sessionId, {
              state: nextState as SocraticState,
              depthLevel,
              qualityScore,
              bktDelta: newBKTState.pL - currentBKT.pL,
              misconception: suvResult.misconception || null,
              turnNumber,
            }).catch(console.error)
            ...
            const newSessionState: SessionState = {
              ...
              bktPL: newBKTState.pL,
              bktPT: newBKTState.pT,
              bktPS: newBKTState.pS,
              bktPG: newBKTState.pG,
            }
